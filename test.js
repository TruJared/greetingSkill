...
const lambdaToTest = require('./index');
...
function validCard(ctx, standardCard) {
  ...
  if (standardCard) {
    ...
    expect(ctx.speechResponse.response.card.text).to.not.be.undefined;
    expect(ctx.speechResponse.response.card.image).to.not.be.undefined;
